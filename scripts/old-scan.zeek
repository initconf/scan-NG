...
export {
	...
	const shut_down_thresh = 19 &redef;
	...
	global check_scan: function(c: connection, established: bool, reverse: bool): bool;
	...
	global scan_summary: function(t: table[addr] of set[addr], orig: addr)
	    : interval;
	...
	global pre_distinct_peers: table[addr] of set[addr] &read_expire=1 days &redef;

	global distinct_peers: table[addr] of set[addr] &read_expire=1 days
	    &expire_func=scan_summary &redef;
	...
	global accounts_tried: table[addr] of set[string, string] &read_expire=1 days;
	...
	global rb_idx: table[addr] of count &default=1 &read_expire=1 days &redef;
	...
	global rat_idx: table[addr] of count &default=1 &read_expire=1 days &redef;
	global rrat_idx: table[addr] of count &default=1 &read_expire=1 days &redef;
}
...
function check_scan(c: connection, established: bool, reverse: bool): bool
{
	...
	( analyze_all_services || service in analyze_services ) ) {
		if ( reverse
		    && rev_service in backscatter_ports
		    && # reverse, non-priv backscatter port
		service >= 1024/tcp ) {
			if ( orig !in distinct_backscatter_peers ) {
				local empty_bs_table: table[addr] of count &default=0;
				distinct_backscatter_peers[orig] = empty_bs_table;
			}

			if ( ++distinct_backscatter_peers[orig][resp] <= 2 && # The test is <= 2 because we get two check_scan()
			...
			thresh_check(report_backscatter, rb_idx, orig,
				...
				NOTICE([
				    $note=BackscatterSeen,
				    $src=orig,
				    $p=rev_service,
				    $msg=fmt("backscatter seen from %s (%d hosts; %s)", orig, |distinct_backscatter_peers[orig]|, rev_svc)]);
			}

			if ( ignore_scanners_threshold > 0
			    && |distinct_backscatter_peers[orig]| > ignore_scanners_threshold )
				ignore_addr(orig);
		} 
		else { # done with backscatter check
			local ignore = F;
			...
			if ( ! ignore ) { # XXXXX
				...
				if ( ! shut_down_thresh_reached[orig]
				    && n >= shut_down_thresh
				    && ! outbound
				    && orig !in Site::neighbor_nets ) # && service != 3389/tcp)
				    {
					...
					if ( address_scan )
						NOTICE([
						    ...
						    $p=service,
						    $n=n,
						    $msg=fmt("%s has scanned %d hosts (%s)", orig, n, svc)]);
	...
	if ( activate_priv_port_check
		...
		if ( orig !in distinct_low_ports || service !in distinct_low_ports[orig] ) {
			...
			if ( |distinct_low_ports[orig]| == priv_scan_trigger
				...
				NOTICE([$note=LowPortTrolling, $src=orig, $p=service, $msg=svrc_msg]);
			}

			if ( ignore_scanners_threshold > 0
			    && |distinct_low_ports[orig]| > ignore_scanners_threshold )
				ignore_addr(orig);
		}
	}
...
event account_tried(c: connection, user: string, passwd: string)
{
	...
	if ( Site::is_local_addr(src) ) {
		...
		if ( thresh_check(report_accounts_tried, rat_idx, src, |accounts_tried[src]|) )
			threshold_check = T;
	}
	...
	if ( threshold_check && src !in skip_accounts_tried ) {
		...
		NOTICE([
		    $note=PasswordGuessing,
		    $src=src,
		    $n=m,
		    $user=user,
		    $sub=passwd,
		    $p=c$id$resp_p,
		    $msg=fmt("%s has tried %d username/password combinations (latest: %s@%s)", src, m, user, c$id$resp_h)]);
	}
...
event login_successful(c: connection, user: string)
{
	...
	if ( src in accounts_tried
		...
		NOTICE([
		    $note=SuccessfulPasswordGuessing,
		    $src=src,
		    $conn=c,
		    $msg=fmt("%s successfully logged in user '%s' after trying %d username/password combinations", src, user, |accounts_tried[src]|)]);
}
...
{
	local is_reverse_scan = ( c$orig$state == TCP_INACTIVE && c$id$resp_p !in
	    likely_server_ports );
...
{
	local is_reverse_scan = ( c$orig$state == TCP_RESET && c$id$resp_p !in
	    likely_server_ports );
