...
event zeek_init()
{
	Broker::auto_publish(Cluster::worker_topic,
	    Scan::m_w_send_performance_counters);
}
@else
event zeek_init()
{
	Broker::auto_publish(Cluster::manager_topic,
	    Scan::w_m_update_performance_counters);
}
@endif
