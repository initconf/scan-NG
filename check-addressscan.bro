...
function check_address_scan_thresholds (orig: addr, resp: addr, outbound: bool, n: count): bool  
{
	...
	if ( (! established ) &&
		...
		if ( ! shut_down_thresh_reached[orig] &&
		     ...
		     ! outbound && orig !in Site::neighbor_nets)
			{
				...
				result = T ;
                                add_to_known_scanners(orig, "ShutdownThresh");
                                Scan::known_scanners[orig]$detect_ts=network_time();
