...
export {
	...
	global expire_worker_stats: function(t: table[addr] of scan_stats, idx: addr)
	    : interval;
	global worker_stats: table[addr] of scan_stats = table()
	    &create_expire=_worker_time &expire_func=expire_worker_stats;

	global report_manager_stats: function(t: table[addr] of scan_stats, idx: addr)
	    : interval;
	global manager_stats: table[addr] of scan_stats = table() &create_expire=20 secs
	    &expire_func=report_manager_stats;
	...
	when ( local myaddrs = lookup_hostname(gethostname()) ) {
		for ( ip in myaddrs ) {
			if ( is_v4_addr(ip) ) {
				...
				if ( myloc?$city ) {
					print fmt("Using %s, %s as local location", myloc$city, myloc$region);
				}
				break;
			}
		}
...
function Scan::expire_worker_stats(t: table[addr] of scan_stats, idx: addr)
    : interval
{
	if ( idx in known_scanners ) {
		#log_reporter(fmt("expire_worker_stats: sending to manager:  %s", t[idx]),10);
@if ( Cluster::is_enabled() )
		local scan_sub=get_subnet(idx);
		Cluster::publish_hrw(Cluster::proxy_pool, scan_sub, Scan::aggregate_scan_stats, t[idx]);
@else
		event Scan::aggregate_scan_stats(t[idx]);
@endif
	} else if ( idx in scan_candidates && network_time() - t[idx]$start_ts <
	    _max_expire_time ) {
...
event connection_state_remove(c: connection)
{
	...
	if ( orig !in worker_stats ) {
		...
		worker_stats[orig]$event_peer = fmt("%s", Cluster::node);
...
function Scan::report_manager_stats(t: table[addr] of scan_stats, idx: addr)
    : interval
{
...
event Scan::aggregate_scan_stats(ss: scan_stats)
{
	...
	manager_stats[orig]$start_ts = ss$start_ts < m_start_ts ? ss$start_ts :
	    m_start_ts;
	...
	manager_stats[orig]$event_peer = fmt("%s", peer_description);
...
function log_scan_summary(ss: scan_stats, state: log_state)
{
	...
	info$scan_rate = info$total_hosts_scanned == 0 ? 0 : interval_to_double(
	    info$duration) / info$total_hosts_scanned;
