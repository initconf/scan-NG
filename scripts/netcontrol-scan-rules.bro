...
event NetControl::rule_timeout(r: Rule, i: FlowInfo, p: PluginState) &priority=-5
        {
	local ip = subnet_to_addr(r$entity$ip) ; 
	 Scan::log_reporter(fmt ("acld_rule_timeout: Rule: %s, %s", subnet_to_addr(r$entity$ip), r),1);
        }
