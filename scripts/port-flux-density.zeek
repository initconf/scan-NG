...
export {
	...
	global concurrent_scanners_per_port: table[port] of opaque of cardinality 
        ...
        global check_flux_density_threshold: function (v: vector of count, idx: table[port] of count, orig: port, n: count):bool ;

	global check_port_flux_density: function(p : port, a: addr): count ; 	

} 

function check_flux_density_threshold(v: vector of count, idx: table[port] of count, orig: port, n: count):bool
{
        if (orig !in idx)
                idx[orig]=  0 ;
...
function check_port_flux_density(d_port : port, ip: addr): count 
{
	...
	local d_val = double_to_count(hll_cardinality_estimate(concurrent_scanners_per_port[d_port]));

	local result = check_flux_density_threshold(flux_density_threshold, flux_density_idx, d_port, d_val);
