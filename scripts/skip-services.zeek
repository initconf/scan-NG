...
event port_exclude(description: Input::TableDescription, tpe: Input::Event,
    left: Idx, right: portexclude_Val)
{
	local _msg = "";
...
{
	Input::add_table([
	    $source=portexclude_file,
	    $mode=Input::REREAD,
	    $name="port_exclude",
	    $destination=port_exclude_table,
	    $idx=Idx,
	    $val=portexclude_Val,
	    $ev=port_exclude]);
}
