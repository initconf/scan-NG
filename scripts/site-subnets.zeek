...
export {
	...
	global allocated_cache: set[subnet]
	    &create_expire=WATCH_REMOVED_ALLOCATED_SUBNET
	    &expire_func=expire_allocated_cache;
}
...
{
	if ( n$note == Scan::DarknetSubnetAdded ) {
		add n$actions[Notice::ACTION_LOG];
	}
	if ( n$note == Scan::AllocatedSubnetRemoved ) {
		add n$actions[Notice::ACTION_LOG];
	}
	if ( n$note == Scan::SubnetTableZero ) {
		add n$actions[Notice::ACTION_LOG];
	}
	if ( n$note == Scan::ReactivatingSubnetTable ) {
		add n$actions[Notice::ACTION_LOG];
	}
	if ( n$note == Scan::MissingSubnetsFeed ) {
		add n$actions[Notice::ACTION_LOG];
	}
}
...
event reporter_warning(t: time, msg: string, location: string)
{
	...
	#{
	#for (nets in Scan::local_nets)
	#       	{
	#	#print fmt("nets: %s", nets);
	#	local sv: subnet_Val = [$Network=nets, $Gateway=0.0.0.0, $Enclaves="Site", $Use="Filling the empty subnet table"];
	#      		Scan::subnet_table[nets] = sv ;
	#}
	#}
	}
}

event read_subnet_feed(description: Input::TableDescription, tpe: Input::Event,
    left: subnet_Idx, right: subnet_Val)
{
	...
	if ( tpe == Input::EVENT_REMOVED ) {
		...
		NOTICE([
		    $note=AllocatedSubnetRemoved,
		    $msg=fmt("%s", _msg),
		    $identifier=cat("AllocatedSubnetRemoved"),
		    $suppress_for=6 hrs]);
	}
}
...
event Input::end_of_data(name: string, source: string)
{
	...
	print fmt("digested  %s records in subnet_table", |Scan::subnet_table|);
	if ( subnet_feed in source ) {
		#print fmt ("1. name is %s source is %s", name, source);
		#print fmt("2. digested  %s records in %s",|Scan::subnet_table|, source);

		# since subnet table is zero size
		# we poulate with local_nets

		if ( |Scan::subnet_table| == 0 ) {
			_msg = fmt("Looks like subnet Table is %s", |Scan::subnet_table|);
			...
			SubnetCountToActivteLandMine = |Scan::subnet_table|;

		if ( |Scan::subnet_table| > |Site::local_nets| ) {
			# FIXME
			#for (nets in Scan::local_nets)
			#{
...
event zeek_done()
{
	...
	#print fmt("digested  %s records in subnet_table", |Scan::subnet_table|);
	#print fmt("subnet_table %s", Scan::subnet_table);
}
