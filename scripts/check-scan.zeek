...
function not_scanner(cid: conn_id): bool
{
	...
	if ( orig in Scan::whitelist )
		return T;

@ifdef (YURT::CLOUDFLARE)
	if ( orig in YURT::CLOUDFLARE)
		return T;
@endif
	...
	# this is a big sword - we should not blanket
	# skip but rather do it per heuristic
	...
	# Lets ignore everything which has an outgoing established
	# connection from us to a given subnet
	# this is somewhat a big blanket of ignore

	#seen = bloomfilter_lookup(Scan::outgoing_SF, orig);
	#if (seen == 1 )
	#	return T;
...
function check_scan(c: connection, established: bool, reverse: bool)
{
	...
	if (orig in hs || isHotSubnet(orig))
		darknet = T ;
	...
	else if ( Site::is_local_addr(resp) && resp in Scan::subnet_table ) {
		if ( gather_statistics )
	...
	if ( activate_Backscatter && ! uid_table[c$uid] )
		filter__Backscatter = Scan::filterate_Backscatter(c, darknet);

	if ( activate_KnockKnockScan && ! uid_table[c$uid] )
		filter__KnockKnock = Scan::filterate_KnockKnockScan(c, darknet);

	if ( activate_SubnetKnock && ! uid_table[c$uid] )
		filter__SubnetKnock = Scan::filterate_SubnetKnock(c, darknet);
	...
	if ( activate_PortScan && ! uid_table[c$uid] )
		filter__PortScan = Scan::filterate_PortScan(c, established, reverse);


	if (   /K/ in filter__KnockKnock
	    || /S/ in filter__SubnetKnock
	    || /L/ in filter__LandMine
	    || /B/ in filter__Backscatter
	    || /A/ in filter__AddressScan
	    || /T/ in filter__LowPortTroll
	    || /P/ in filter__PortScan ) {
		...
		if ( c$uid !in uid_table ) {
			local filterator = fmt("%s%s%s%s%s%s%s", filter__KnockKnock, filter__LandMine,
			    filter__Backscatter, filter__AddressScan,
			    filter__PortScan, filter__LowPortTroll,filter__SubnetKnock);
			uid_table[c$uid] = T;
...
event new_connection(c: connection)
{
	...
	if ( ( tp == tcp || tp == udp ) && c$id$orig_h !in Site::local_nets &&
	    (is_darknet(c$id$resp_h) || isHotSubnet(c$id$orig_h)) ) {
	#print fmt ("-1: event new_connection (c: connection): %s", c$id);
...
event connection_state_remove(c: connection)
{
	...
	if ( (( trans == udp ) || (trans == tcp) || (trans == icmp ))
	&& (c$conn$conn_state != "SF")
	&& ( /d|D/ !in c$history ) ) {
...
event connection_established(c: connection)
{
	...
	local is_reverse_scan = ( c$orig$state == TCP_INACTIVE && c$id$resp_p !in
	    likely_server_ports );
	#print fmt ("0. event connection_established (c: connection): %s", c$id); 
