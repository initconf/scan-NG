...
event Scan::line(description: Input::TableDescription, tpe: Input::Event, left: blocknet_Idx, right: blocknet_Val)
{
        local msg: string;
...
{
        Input::add_table([$source=blocknet_feed, $name="blocked_nets", $idx=blocknet_Idx, $val=blocknet_Val,  $destination=blocked_nets, $mode=Input::REREAD, $ev=Scan::line]);
